...
// #########################################################################
// Draw 5-points meter at x, y and define full scale range & the scale labels
// #########################################################################
/*
...
void MeterWidget::analogMeter(uint16_t x, uint16_t y, float startScale, float endScale, const char *units, const char *s0, const char *s1, const char *s2, const char *s3, const char *s4)
{
  ...
  // Meter outline
  if (_draw_outline)  ntft->fillRect(x, y, 239, 126, _outline_color);
  ntft->fillRect(x + 5, y + 3, 230, 119, _background_color);

  ntft->setTextColor(_text_color);  // Text colour

  // Draw ticks every 5 degrees from -50 to +50 degrees (100 deg. FSD swing)
  for (int i = -50; i < 51; i += 5) {
    // Long scale tick length
    int tl = _long_tick_length;
    ...
    if (i % 25 != 0) tl = _short_tick_length;
    ...
    if (i % 25 == 0) {
      ...
      x0 = x + sx * (100 + tl + 15) + 120;
      y0 = y + sy * (100 + tl + 15) + 140;
      switch (i / 25) {
  ...
  if (_draw_small_label) ntft->drawRightString(slabel, mx + 230, my + 102, 2);	// Small label for additional info
  if (_draw_bezel)  ntft->drawRect(x + 5, y + 3, 230, 119, _bezel_color);           // Draw bezel line
...
// 4 points meter
void MeterWidget::analogMeter(uint16_t x, uint16_t y, float fullScale, const char *units, const char *s0, const char *s1, const char *s2, const char *s3) 
{
	analogMeter(x, y, 0.0, fullScale, units, s0, s1, s2, s3);
}
void MeterWidget::analogMeter(uint16_t x, uint16_t y, float startScale, float endScale, const char *units, const char *s0, const char *s1, const char *s2, const char *s3) 
{
  _num_points = 4;
  strncpy(ms0, s0, 4);
  strncpy(ms1, s1, 4);
...
void MeterWidget::updateNeedle(float val, uint32_t ms_delay)
{
  ...
  char buf[8]; dtostrf(val, 5, _digital_precision, buf);
  ...
  while (value != old_analog) 
  {
	...
	if (_use_60_deg) sdeg = map(old_analog, -10, 110, -160, -20); // 120 degrees swing +/- additional 10 degrees
	else sdeg = map(old_analog, -10, 110, -150, -30); // 100 degrees swing +/- additional 10 degrees
	float rad = sdeg * 0.0174532925; // Current needle angle in radians
	
    // Calculate x delta of needle start (does not start at pivot point)
    float tx = tan((sdeg + 90) * 0.0174532925);		// old way
    
    // Tip of needle (Radius 98)
    float sx = cos(rad);
    float sy = sin(rad);
    
	// Base of needle (Radius 20) - Matches the pivot point
    float bx = sx * 20; 
    float by = sy * 20;
	...
	// 2. ERASE OLD NEEDLE
	    for (int i = -2; i <= 2; i++) 
	{
		ntft->drawLine(mx + 120 + 20 * ltx + i, my + 140 - 20, mx + osx + i, my + osy, _background_color); 	// old way erasure
		// ntft->drawLine(mx + 120 + obx + i, my + 140 + oby, mx + osx + i, my + osy, _background_color);
    }

	// 3. RE-PLOT TEXT (In case needle passed over it)
    ntft->setTextColor(_text_color);
    ntft->drawCentreString(mlabel, mx + 120, my + 70, 4);

	// 4. UPDATE SAVED COORDS FOR NEXT ERASE
	ltx = tx; // old way
    osx = sx * 98 + 120;
    osy = sy * 98 + 140;
    obx = bx;
	...
	// 5. DRAW NEW NEEDLE
    int thickness = _draw_thicker_needle ? 2 : 1;
    for (int i = -thickness; i <= thickness; i++) 
	{
        // ntft->drawLine(mx + 120 + bx + i, my + 140 + by, mx + osx + i, my + osy, _needle_color); 	
		ntft->drawLine(mx + 120 + 20 * ltx + i, my + 140 - 20, mx + osx + i, my + osy, _needle_color);	// old way render
    }

	/*
	// muffled for now as it's a little buggy
	uint8_t capRadius = 20; // Adjust for the size you like
	ntft->fillCircle(mx + 120, my + 140, capRadius, _needle_color);
	ntft->fillRect(100, 195, 50, 40, _text_color);
	*/
...
// Helper to draw the labels by index
void MeterWidget::drawLabelByIndex(int index, int x, int y) 
{
  const char* label;
  switch (index) 
  {
    case 0: label = ms0; break;
    case 1: label = ms1; break;
    case 2: label = ms2; break;
    case 3: label = ms3; break;
    case 4: label = ms4; break;
    case 5: label = ms5; break;
    default: break;
  }
  ntft->drawCentreString(label, x, y - 6, 2);
}
...
void MeterWidget::drawMeterFace(uint16_t x, uint16_t y, float startScale, float endScale, const char *units) 
{
  ...
  // Auto-calculate interval (120 degree span / gaps)
  const uint8_t _full_swing_angle = 100;
  uint8_t labelInterval = _full_swing_angle / (_num_points - 1); 
  ...
  for (int8_t i = -(_full_swing_angle/2); i <= (_full_swing_angle/2) + 1; i += 5) 
  {
		...
		// Short scale tick length
		if (i % (_full_swing_angle / 4) != 0) tl = _short_tick_length;
